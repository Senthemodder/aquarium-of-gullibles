 ...
 */
export function validateCommandPermission(origin, requiredLevel = CommandPermissionLevel.Admin) {
    if (origin.permissionLevel !== undefined && origin.permissionLevel < requiredLevel) {
        return false;
    }
    if (origin.sourceEntity?.permissionLevel !== undefined &&
 ...
 */
export function executeInspectCommand(origin, requiredLevel = CommandPermissionLevel.Admin) {
...
export function registerInspectCommand(eventOrRegistry) {
    ...
    if (!registry || typeof registry.registerCommand !== 'function') {
        throw new TypeError('CustomCommandRegistry instance with registerCommand function is required.');
    }
    ...
    registry.registerCommand(inspectCommandDefinition, (origin) => {
        return executeInspectCommand(origin, CommandPermissionLevel.Admin);
    });
